function [abl] = toscaDataEvaluateFromProbes(abl)
...
% read data from the files generated with TOSCA code
load('./input/gridProbes_U.mat');
load('./input/ABL_averaging.mat');

% resample on the shorter vertical grid 
z_probes   = gridProbes.zArray;
z_averages = averages.levels;

zs         = max([min(z_probes), min(z_averages)]);
ze         = min(max(z_probes), max(z_averages));
z          = (zs:1:ze)';

% shear stress profile 
Nt_avg        = size(averages.U_mean.values, 1);
Nz_avg        = size(averages.U_mean.values, 2);
...
% resample on z 
wu = interp1(z_averages, wu, z);
wv = interp1(z_averages, wv, z);

% velocity profile 
Nz_probes = length(z_probes);
u         = zeros(1, Nz_probes);
v         = zeros(1, Nz_probes);

for t = 1:Nz_probes
    u(t)  = mean(gridProbes.X(1,:,t));
    v(t)  = mean(gridProbes.Y(1,:,t));
end

% resample on z
u = interp1(z_probes, u, z);
v = interp1(z_probes, v, z);
